...
export default function ProductCard({ id, name, price, image, isSoldOut, link }: ProductCardProps) {
  return (
    <div className="group flex flex-col h-full bg-white hover:shadow-xl transition-shadow duration-300 border border-transparent hover:border-gray-100">
      {/* Image Container */}
      <Link href={link}>
        <div className="relative aspect-[4/3] overflow-hidden bg-gray-50 flex items-center justify-center p-6 cursor-pointer">
          {isSoldOut && (
            <div className="absolute top-4 right-4 bg-red-600 text-white text-xs font-bold px-3 py-1 uppercase tracking-wider z-10">
              Épuisé
            </div>
          )}
          <img 
